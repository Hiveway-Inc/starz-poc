...
function setSessionDetails(session) {
  ...
  switch (session.status) {
    case "complete":
      statusText = "Subscription started";
      iconColor = "#30B130";
      icon = SuccessIcon;
      break;
    case "open":
      statusText = "Checkout incomplete";
      iconColor = "#DF1B41";
      icon = ErrorIcon;
      break;
    default:
  ...
  if (session.subscription_id) {
    viewDetailsLink.href = `https://dashboard.stripe.com/subscriptions/${session.subscription_id}`;
  } else {
    viewDetailsLink.classList.add("hidden");
  }
}
